...
import * as previous from './engine-v3.ts';
import {courseFor,courseObstacles,distance,speed} from './course-v4.ts';
export {distance,speed,stageAt,THEMES,corridor,gateState,courseFor,laneAt} from './course-v4.ts';
export const VERSION = 'slip-4';
...
export function createSimulation(config:Config):{shots:Shot[];shapes:(t:number)=>Shape[];step:(a:Point,b:Point,tick:number)=>boolean}{
 ...
 function shapes(t:number):Shape[]{
  ...
  for(const shot of shots){const age=t-shot.fireTick/HZ;if(age<0||age>2.5)continue;
   ...
   base.push({id:shot.row+':shot:'+shot.burst,x:x-11,y:y-4,w:22,h:8,round:false,coral:false,color:'#ea4765',kind:'bolt',angle:Math.atan2(shot.dy+speed(t),shot.dx),row:shot.row});
  }return base;
 }
 return {shots,shapes,step(a:Point,b:Point,tick:number){
  ...
  if(next<turrets.length&&tick===turrets[next].aimTick){
   const turret=turrets[next++],c=turret.centre,side=c>180?-1:1,cx=c+side*99;
