import type {Point,Shape} from './engine-v4.ts';
const W=360,H=640,BARRIER=4;
export const THEMES=[
 {name:'GATEWAY',color:'#3676ed',colors:['#3676ed','#ff655b','#7960e9','#13b5ba','#ffb62d','#ec5197','#79bb3b']},
 {name:'CONDUIT',color:'#009b9b',colors:['#009b9b','#3676ed','#19b98d']},
 {name:'SWITCHBACK',color:'#7950db',colors:['#7950db','#ce49a0','#3676ed']},
 {name:'CROSSFIRE',color:'#e56b28',colors:['#e56b28','#ed526a','#e9a822']},
 {name:'OVERDRIVE',color:'#2455d6',colors:['#2455d6','#13aaad','#7950db']},
] as const;
...
export function speed(t:number){return 155+90*(1-Math.exp(-t/45))+240*(1-Math.exp(-t/200));}
export function distance(t:number){return 155*t+90*(t-45*(1-Math.exp(-t/45)))+240*(t-200*(1-Math.exp(-t/200)));}
...
const sequences:Scene['type'][][]=[['gate','turret','rotor','gate','gem','gate','rotor'],['tunnel','gate','tunnel','rotor','gate'],['maze','gate','maze','rotor'],['turret','gate','turret','rotor','gate','maze'],['tunnel','rotor','maze','gate','turret']];
const courses=new Map<number,Scene[]>();
export function laneAt(scene:Scene,u:number){
 if(!scene.knots.length)return {centre:scene.centre,half:64};
...
export function courseFor(seed:number){
 ...
 for(let stage=0;stage<12;stage++){
  ...
  for(let n=0;cursor+420<=end;n++){
   let type=seq[n%seq.length],depth=type==='tunnel'?1320:type==='maze'?1560:160,span=type==='turret'?1120:depth+330;
   if(cursor+span>end){type='gate';depth=160;span=490;if(cursor+span>end)break;}
   const phase=rand(seed,id+10)*Math.PI*2,centre=180+70*Math.sin(phase),knots=type==='tunnel'?[180,180+65*Math.sin(phase),180-65*Math.sin(phase),180]:type==='maze'?[180,110,250,110,250,180]:[];
...
export function gateState(t:number,row:number,seed=0){
 const scene=courseFor(seed)[row],age=Math.max(0,t-arrival((scene?.start||0)+140)),cycle=age%5.2;
 ...
 for(const scene of visibleScenes(t,seed)){
  ...
  if(scene.type==='gate'||scene.walls){
   ...
   for(const sign of [-1,1]){const rail=sign<0?c.centre-c.half-BARRIER:W-BARRIER-c.centre-c.half,w=rail*state.extension;
    const shape=add(scene.id+':gate:'+sign,sign<0?BARRIER+w/2:W-BARRIER-w/2,front,w,22,'gate',scene);
    shape.rail=rail;shape.side=sign;shape.warning=state.warning;shape.solid=w>.01;
   }
  }else if(scene.type==='turret'){
   const shape=add(scene.id+':shooter',c.centre+side*(c.half+35),front,58,58,'shooter',scene);shape.spent=t>(scene.aimTick+104)/120;
  }else if(scene.type==='rotor'){
   add(scene.id+':rotor',c.centre+side*(c.half+86),front,160,22,'bar',scene,t*.48+scene.phase);
  }else add(scene.id+':gem',c.centre+side*(c.half+51),front,92,92,'polygon',scene,-t*.38+scene.phase);
 }
 return out;
}
